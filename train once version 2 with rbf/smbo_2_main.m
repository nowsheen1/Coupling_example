...
conffn = str2func('conf2');  % Obtain function handle for configurations
...
confn1 = @(xopt)con3(xopt);
confn2 = @(xopt)con4(xopt);
...
while (k <= maxiter)
    ...
    switch(m.surrogate)
        case 'TPS-RBF'      % Radial-Basis Function (Thin-Plate Spline)
            ...
            [weight_2,center_2] = tps_rbf_construct(xs_g,fsmp);
            ...
            [xopt,fopt] = ga(@(x)tps_rbf_objfn(x,weight_2,center_2),pc.nvar,...
  ...
  %  if visualization_on
   %     outputfn_10;        % Command window output per each iteration
    ...
    %end
    save('store_surrogate_data2.mat', 'store', '-v7.3');
    save('store_optimal_data2.mat', 'optimal', '-v7.3');
