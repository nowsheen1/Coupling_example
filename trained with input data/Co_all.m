...
function X_Sub_sys_1=Sub_system_1_opt(xs)
FUN = @Sub_sys_1_obj;
X0 = [1;1;1]; 
A = [];
B = [];
...
Coupling0=[2;4];    
t=sys_eqn_solv_couplingvar( x,Coupling0 );
f=0.5*(((x(1)-xs))^2+(((x(1))^2+2*x(2)-x(3)+2*sqrt(t(2)))-t(1))^2);
end
function [c,ceq]=constraints_Sub_sys_1(x)
Coupling0=[2;4];    
t=sys_eqn_solv_couplingvar( x,Coupling0 );
c(1)=2-((x(1))^2+2*x(2)+x(3)+x(2)*exp(-t(2)));
c(2)=((x(1))^2+2*x(2)+x(3)+x(2)*exp(-t(2)))-20;
ceq=[];
end
...
[u,fval1,exitflag1,output1] = fsolve(@equations, Coupling0, options);  
 ...
 function f = equations(Coupling)
    ...
    f =[sys_eqn_solv_couplingvar1( x,Coupling );...
