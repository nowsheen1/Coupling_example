...
maxiter = 60;               % Maximum number of iterations allowed
n_smp = 30;                  % Number of samples for each iteration
...
objfn = @(xopt)obj(xopt);
confn1 = @(xopt)con1(xopt);
confn2 = @(xopt)con2(xopt);
...
while (k <= maxiter)
    ...
    %%
      %%constraint surrogate2
      f_hf_con2{k} = zeros(n_smp,1); %  for one iteration the size of f is 30*1
    for i = 1:n_smp     % If objfn is computationally expensive, this is 
                        % the most computationally intensive step.
        f_hf_con2{k}(i) = feval(confn2,x_sample{k}(i,:));
    end
    
    % ==== SURROGATE MODELING-BASED OPTIMIZATION ====
    % Compile design points (past and current samples) along with high
    % fidelity function results:
    xs_g_con2 = x_sample{1};     % For first iteration, we only have sample pts
    fsmp_con2 = f_hf_con2{1};         % and its high fidelity function results
    if (k>1)
        for i = 2:k
            xs_g_con2 = [xs_g_con2; x_sample{i}; xopt_history{i-1}];
            fsmp_con2 = [fsmp_con2; f_hf_con2{i}; f_hf_opt_con2_history{i-1}];
        end
    end
    ...
    switch(m.surrogate)
        case 'TPS-RBF'      % Radial-Basis Function (Thin-Plate Spline)
            ...
            [weight2,center2] = tps_rbf_construct( xs_g_con2, fsmp_con2);
            % Run GA-based optimization using the surrogate model
            [xopt,fopt] = ga(@(x)tps_rbf_objfn(x,weight,center),pc.nvar,...
                [],[],[],[],pc.lb,pc.ub,@(x)tps_rbf_confn(x,weight1,center1,weight2,center2),opt);
    ...
    f_hf_opt_con2_history{k} = f_hf_opt_con2;
...
for i = 1:maxiter
    ...
    x_table=store{i};
    x_table_input=x_table(:,4:6);

    no_row=size(x_table_input,1);
    opt_sol=repmat(optimal{i},no_row,1);

    reconfiguredCell = [reconfiguredCell; store{i}(:, 4:6)];
    opt_sol_con=[opt_sol_con;opt_sol];

end
model_frst_optimal_soln_sub1=fitrgp(reconfiguredCell, opt_sol_con(:,1));
model_second_optimal_soln_sub1=fitrgp(reconfiguredCell, opt_sol_con(:,2));
model_third_optimal_soln_sub1=fitrgp(reconfiguredCell, opt_sol_con(:,3));
save('gprModel_sub11.mat', 'model_frst_optimal_soln_sub1');
save('gprModel_sub12.mat', 'model_second_optimal_soln_sub1');
save('gprModel_sub13.mat', 'model_third_optimal_soln_sub1');
z=[xopt,fopt];
end
